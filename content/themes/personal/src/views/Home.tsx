...
export default class Home extends View<{opened: boolean, posts: PostInterface[] | null, projects: ProjectInterface[] | null, mouseMode: boolean}> {
    ...
    private dragRender = () => this.dragRenderOpts();
    ...
    private onResize = () => {
        ...
        if (this.content) {
            this.content.style.transform = this.state!!.mouseMode ? `translateY(${this.top}px)` : null;
        }
        this.updateHeight();
    }
    private attachWrapper = (el) => {
    ...
    public render() {
        ...
        const reallyFastConnection = (downlink >= 7);
        ...
        return (<div className={'home-component' + (mouseMode ? ' mousemode' : '')} ref={this.attachWrapper}>
            <div className={'home-video-wrapper' + (opened ? ' opened' : '') }>
                <video loop="loop" preload="auto" autoplay="autoplay" data-object-fit="cover" muted="muted" className="home-video" playsinline ref={this.attachVideo}>
                    <source src={'/assets/home-video' + (assumeWifi ? (reallyFastConnection ? '-desktop' : '') : '-mobile') + '.mp4'} type="video/mp4" />
                </video>
            </div>
